import { Ct as registerPreprocessor, Do as each, Jr as ZRText, Po as isArray, Pr as enableHoverEmphasis, Q as createDimensions, T as createTextStyle, Tn as getLayoutRect, Ui as linearMap, d as extendChartView, et as SeriesData, kn as capitalFirst, m as extendSeriesModel, n as install, r as installLabelLayout, rt as use, t as install$1, vt as registerLayout } from "./install-D2aADMtJ.js";
//#region node_modules/echarts/lib/echarts.js
use([install, install$1]);
use(installLabelLayout);
//#endregion
//#region node_modules/echarts-wordcloud/src/WordCloudSeries.js
extendSeriesModel({
	type: "series.wordCloud",
	visualStyleAccessPath: "textStyle",
	visualStyleMapper: function(model) {
		return { fill: model.get("color") };
	},
	visualDrawType: "fill",
	...
	getInitialData: function(option, ecModel) {
		var list = new SeriesData(createDimensions(option.data, { coordDimensions: ["value"] }), this);
...
//#endregion
//#region node_modules/echarts-wordcloud/src/WordCloudView.js
extendChartView({
	type: "wordCloud",
	render: function(seriesModel, ecModel, api) {
		...
		seriesModel.layoutInstance.ondraw = function(text, size, dataIdx, drawn) {
			...
			var textEl = new ZRText({
				style: createTextStyle(itemModel.getModel("textStyle")),
			...
			textEl.setStyle({
				...
				y: drawn.info.fillTextOffsetY + size * .5,
				text,
			...
			textEl.ensureState("emphasis").style = createTextStyle(itemModel.getModel(["emphasis", "textStyle"]), { state: "emphasis" });
			textEl.ensureState("blur").style = createTextStyle(itemModel.getModel(["blur", "textStyle"]), { state: "blur" });
			enableHoverEmphasis(textEl, itemModel.get(["emphasis", "focus"]), itemModel.get(["emphasis", "blurScope"]));
...
//#endregion
//#region node_modules/echarts-wordcloud/src/layout.js
/*!
* wordcloud2.js
...
*/
if (!window.setImmediate) window.setImmediate = (function setupSetImmediate() {
	return window.msSetImmediate || window.webkitSetImmediate || window.mozSetImmediate || window.oSetImmediate || (function setupSetZeroTimeout() {
		...
		window.addEventListener("message", function setZeroTimeoutMessage(evt) {
			if (typeof evt.data !== "string" || evt.data.substr(0, message.length) !== message) return;
...
var minFontSize = (function getMinFontSize() {
	...
	while (size) {
		...
		if (ctx.measureText("Ｗ").width === hanWidth && ctx.measureText("m").width === mWidth) return size + 1;
...
var WordCloud = function WordCloud(elements, options) {
	...
	elements.forEach(function(el, i) {
		if (typeof el === "string") {
			...
			if (!elements[i]) throw new Error("The element id specified is not found.");
		} else if (!el.tagName && !el.appendChild) throw new Error("You must pass valid HTML elements, or ID of the element.");
	});
	var settings = {
		list: [],
	...
	if (options) {
		for (var key in options) if (key in settings) settings[key] = options[key];
	}
	...
	if (typeof settings.shape !== "function") switch (settings.shape) {
		case "circle":
		default:
		...
		case "pentagon":
			settings.shape = function shapePentagon(theta) {
				var thetaPrime = (theta + .955) % (2 * Math.PI / 5);
				return 1 / (Math.cos(thetaPrime) + .726543 * Math.sin(thetaPrime));
			};
			break;
		case "star":
			settings.shape = function shapeStar(theta) {
				var thetaPrime = (theta + .955) % (2 * Math.PI / 10);
				if ((theta + .955) % (2 * Math.PI / 5) - 2 * Math.PI / 10 >= 0) return 1 / (Math.cos(2 * Math.PI / 10 - thetaPrime) + 3.07768 * Math.sin(2 * Math.PI / 10 - thetaPrime));
				else return 1 / (Math.cos(thetaPrime) + 3.07768 * Math.sin(thetaPrime));
			};
			break;
	}
	...
	switch (settings.color) {
		...
		case "random-light":
			...
			break;
		default:
			if (typeof settings.color === "function") getTextColor = settings.color;
			break;
	}
	var getTextFontWeight;
	if (typeof settings.fontWeight === "function") getTextFontWeight = settings.fontWeight;
	var getTextClasses = null;
	if (typeof settings.classes === "function") getTextClasses = settings.classes;
	...
	var getInfoGridFromMouseTouchEvent = function getInfoGridFromMouseTouchEvent(evt) {
		...
		if (!infoGrid[x]) return null;
		return infoGrid[x][y];
	};
	var wordcloudhover = function wordcloudhover(evt) {
		var info = getInfoGridFromMouseTouchEvent(evt);
		if (hovered === info) return;
		hovered = info;
		if (!info) {
	...
	var getPointsAtRadius = function getPointsAtRadius(radius) {
		if (pointsAtRadius[radius]) return pointsAtRadius[radius];
		var T = radius * 8;
		var t = T;
		var points = [];
		if (radius === 0) points.push([
			center[0],
			center[1],
			0
		]);
		while (t--) {
			var rx = 1;
			if (settings.shape !== "circle") rx = settings.shape(t / T * 2 * Math.PI);
			points.push([
	...
	var getRotateDeg = function getRotateDeg() {
		if (settings.rotateRatio === 0) return 0;
	...
	var getTextInfo = function getTextInfo(word, weight, rotateDeg, extraDataArray) {
		...
		if (fontSize <= settings.minSize) return false;
		var mu = 1;
		if (fontSize < minFontSize) mu = (function calculateScaleFactor() {
			var mu = 2;
			while (mu * fontSize < minFontSize) mu += 2;
			return mu;
		})();
		var fontWeight;
		if (getTextFontWeight) fontWeight = getTextFontWeight(word, weight, fontSize, extraDataArray);
		else fontWeight = settings.fontWeight;
		...
		fctx.fillText(word, fillTextOffsetX * mu, (fillTextOffsetY + fontSize * .5) * mu);
		...
		while (gx--) {
			...
			while (gy--) {
				...
				singleGridLoop: while (y--) {
					x = g;
					while (x--) if (imageData[((gy * g + y) * width + (gx * g + x)) * 4 + 3]) {
						occupied.push([gx, gy]);
						if (gx < bounds[3]) bounds[3] = gx;
						if (gx > bounds[1]) bounds[1] = gx;
						if (gy < bounds[0]) bounds[0] = gy;
						if (gy > bounds[2]) bounds[2] = gy;
						if (debug) {
							fctx.fillStyle = "rgba(255, 0, 0, 0.5)";
							fctx.fillRect(gx * g, gy * g, g - .5, g - .5);
						}
	...
	var canFitText = function canFitText(gx, gy, gw, gh, occupied) {
		...
		while (i--) {
			...
			if (px >= ngx || py >= ngy || px < 0 || py < 0) {
				if (!settings.drawOutOfBound) return false;
				continue;
			}
	...
	var drawText = function drawText(gx, gy, info, word, weight, distance, theta, rotateDeg, attributes, extraDataArray) {
		...
		var color;
		if (getTextColor) color = getTextColor(word, weight, fontSize, distance, theta, extraDataArray);
		else color = settings.color;
		var fontWeight;
		if (getTextFontWeight) fontWeight = getTextFontWeight(word, weight, fontSize, extraDataArray);
		else fontWeight = settings.fontWeight;
		var classes;
		if (getTextClasses) classes = getTextClasses(word, weight, fontSize, extraDataArray);
		else classes = settings.classes;
		elements.forEach(function(el) {
			if (el.getContext) {
				...
				if (rotateDeg !== 0) ctx.rotate(-rotateDeg);
				ctx.textBaseline = "middle";
				ctx.fillText(word, info.fillTextOffsetX * mu, (info.fillTextOffsetY + fontSize * .5) * mu);
				ctx.restore();
			} else {
				var span = document.createElement("span");
				var transformRule = "";
				transformRule = "rotate(" + -rotateDeg / Math.PI * 180 + "deg) ";
				if (info.mu !== 1) transformRule += "translateX(-" + info.fillTextWidth / 4 + "px) scale(" + 1 / info.mu + ")";
				var styleRules = {
					position: "absolute",
					display: "block",
				...
				for (var cssProp in styleRules) span.style[cssProp] = styleRules[cssProp];
	...
	var fillGridAt = function fillGridAt(x, y, drawMask, dimension, item) {
		if (x >= ngx || y >= ngy || x < 0 || y < 0) return;
		grid[x][y] = false;
		if (drawMask) elements[0].getContext("2d").fillRect(x * g, y * g, maskRectWidth, maskRectWidth);
		if (interactive) infoGrid[x][y] = {
			item,
			dimension
		};
	};
	var updateGrid = function updateGrid(gx, gy, gw, gh, info, item) {
		...
		while (i--) {
			var px = gx + occupied[i][0];
			var py = gy + occupied[i][1];
			if (px >= ngx || py >= ngy || px < 0 || py < 0) continue;
			fillGridAt(px, py, drawMask, dimension, item);
		}
		if (drawMask) ctx.restore();
	};
	var putWord = function putWord(item, loopIndex) {
		if (loopIndex > 20) return null;
		...
		if (!settings.drawOutOfBound && !settings.shrinkToFit) {
			var bounds = info.bounds;
			if (bounds[1] - bounds[3] + 1 > ngx || bounds[2] - bounds[0] + 1 > ngy) return false;
		}
		var r = maxRadius + 1;
		var tryToPutWordAtPoint = function(gxy) {
			...
			var gw = info.gw;
			var gh = info.gh;
			if (!canFitText(gx, gy, gw, gh, info.occupied)) return false;
		...
		if (settings.shrinkToFit) {
			if (Array.isArray(item)) item[1] = item[1] * 3 / 4;
			else item.weight = item.weight * 3 / 4;
			return putWord(item, loopIndex + 1);
		}
		return null;
	};
	var sendEvent = function sendEvent(type, cancelable, details) {
		if (cancelable) return !elements.some(function(el) {
			var event = new CustomEvent(type, { detail: details || {} });
			return !el.dispatchEvent(event);
		}, this);
		else elements.forEach(function(el) {
			var event = new CustomEvent(type, { detail: details || {} });
			...
			while (gx--) {
				...
				while (gy--) {
					...
					singleGridLoop: while (y--) {
						...
						while (x--) {
							i = 4;
							while (i--) if (imageData[((gy * g + y) * ngx * g + (gx * g + x)) * 4 + i] !== bgPixel[i]) {
								grid[gx][gy] = false;
								break singleGridLoop;
							}
						}
...
//#endregion
//#region node_modules/echarts-wordcloud/src/wordCloud.js
if (!WordCloud.isSupported) throw new Error("Sorry your browser not support wordCloud");
...
registerLayout(function(ecModel, api) {
	ecModel.eachSeriesByType("wordCloud", function(seriesModel) {
		var gridRect = getLayoutRect(seriesModel.getBoxLayoutParams(), {
			width: api.getWidth(),
		...
		WordCloud(canvas, {
			list: data.mapArray("value", function(value, idx) {
				...
				return [
					data.getName(idx),
					itemModel.get("textStyle.fontSize", true) || linearMap(value, valueExtent, sizeRange),
					idx
				];
			}).sort(function(a, b) {
				return b[1] - a[1];
		...
		if (seriesModel.layoutInstance) seriesModel.layoutInstance.dispose();
...
registerPreprocessor(function(option) {
	...
	function compatTextStyle(textStyle) {
		textStyle && each(compats, function(key) {
			if (textStyle.hasOwnProperty(key)) textStyle["text" + capitalFirst(key)] = textStyle[key];
		});
	}
});
