...
var BoundingRect = function() {
	...
	BoundingRect.prototype.union = function(other) {
		...
		if (isFinite(this.x) && isFinite(this.width)) this.width = mathMax$4(other.x + other.width, this.x + this.width) - x;
		else this.width = other.width;
		if (isFinite(this.y) && isFinite(this.height)) this.height = mathMax$4(other.y + other.height, this.y + this.height) - y;
	...
	BoundingRect.prototype.intersect = function(b, mtv) {
		...
		if (mtv) {
			...
			if (ax1 < bx0 || bx1 < ax0) {
				if (dx > dMax) {
					dMax = dx;
					if (d0 < d1) Point.set(maxTv$1, -d0, 0);
					else Point.set(maxTv$1, d1, 0);
				}
			} else if (dx < dMin) {
				dMin = dx;
				if (d0 < d1) Point.set(minTv$1, d0, 0);
				else Point.set(minTv$1, -d1, 0);
			}
			if (ay1 < by0 || by1 < ay0) {
				if (dy > dMax) {
					dMax = dy;
					if (d2 < d3) Point.set(maxTv$1, 0, -d2);
					else Point.set(maxTv$1, 0, d3);
				}
			} else if (dx < dMin) {
				dMin = dx;
				if (d2 < d3) Point.set(minTv$1, 0, d2);
				else Point.set(minTv$1, 0, -d3);
			}
		}
		if (mtv) Point.copy(mtv, overlap ? minTv$1 : maxTv$1);
		return overlap;
	};
...
function TimSort(array, compare) {
	...
	var stackSize = 0;
	length = array.length;
	if (length < 2 * DEFAULT_TMP_STORAGE_LENGTH) length >>> 1;
	var tmp = [];
	runStart = [];
	runLength = [];
...
var localNativeListenerNames = (function() {
	...
	return {
		...
		pointer: map$1(mouseHandlerNames, function(name) {
...
function parsePlainText(text, style) {
	...
	if (text && truncate && width != null) {
		...
		for (var i = 0; i < lines.length; i++) lines[i] = truncateSingleLine(lines[i], options);
	}
...
function fromArc(x, y, rx, ry, startAngle, endAngle, anticlockwise, min, max) {
	...
	if (startAngle < 0) startAngle = startAngle + PI2$6;
	endAngle = endAngle % PI2$6;
	if (endAngle < 0) endAngle = endAngle + PI2$6;
	if (startAngle > endAngle && !anticlockwise) endAngle += PI2$6;
	else if (startAngle < endAngle && anticlockwise) startAngle += PI2$6;
	if (anticlockwise) {
		var tmp = endAngle;
		endAngle = startAngle;
		startAngle = tmp;
...
function normalizeArcAngles(angles, anticlockwise) {
	...
	if (newStartAngle < 0) newStartAngle += PI2$5;
	var delta = newStartAngle - angles[0];
	var newEndAngle = angles[1];
	newEndAngle += delta;
	if (!anticlockwise && newEndAngle - newStartAngle >= PI2$5) newEndAngle = newStartAngle + PI2$5;
	else if (anticlockwise && newStartAngle - newEndAngle >= PI2$5) newEndAngle = newStartAngle - PI2$5;
	else if (!anticlockwise && newStartAngle > newEndAngle) newEndAngle = newStartAngle + (PI2$5 - modPI2(newStartAngle - newEndAngle));
	else if (anticlockwise && newStartAngle < newEndAngle) newEndAngle = newStartAngle - (PI2$5 - modPI2(newEndAngle - newStartAngle));
...
function containStroke$1(cx, cy, r, startAngle, endAngle, anticlockwise, lineWidth, x, y) {
	...
	if (Math.abs(startAngle - endAngle) % PI2$3 < 1e-4) return true;
	if (anticlockwise) {
		var tmp = startAngle;
	...
	if (startAngle > endAngle) endAngle += PI2$3;
...
var ZRText = function(_super) {
	...
	ZRText.prototype._updatePlainTexts = function() {
		...
		for (var i = 0; i < textLines.length; i++) {
			...
			textY += lineHeight;
			if (fixedBoundingRect) el.setBoundingRect(new BoundingRect(adjustTextX(subElStyle.x, style.width, subElStyle.textAlign), adjustTextY(subElStyle.y, calculatedLineHeight, subElStyle.textBaseline), contentWidth, calculatedLineHeight));
		}
	};
...
var SYSTEM_LANG = !env.domSupported ? DEFAULT_LOCALE : function() {
	return (document.documentElement.lang || navigator.language || navigator.browserLanguage).toUpperCase().indexOf(LOCALE_ZH) > -1 ? LOCALE_ZH : DEFAULT_LOCALE;
}();
...
function doGuessOrdinal(data, sourceFormat, seriesLayoutBy, dimensionsDefine, startIndex, dimIndex) {
	...
	function detectValue(val) {
		...
		if (val != null && isFinite(val) && val !== "") return beStr ? BE_ORDINAL.Might : BE_ORDINAL.Not;
...
function buildSection(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
	...
	else return wrapBlockHTML("<div style=\"" + nameStyle + ";" + TOOLTIP_LINE_HEIGHT_CSS + ";\">" + encodeHTML(displayableHeader) + "</div>" + subMarkupText, topMarginForOuterGap);
}
...
*/
var version = "5.4.3";
var dependencies = { zrender: "5.4.4" };
...
var fetchers = {
	...
	parallel: function(seriesModel, result, axisMap, categoryAxisMap) {
		...
		each$2(parallelModel.parallelAxisIndex, function(axisIndex, index) {
...
function injectOrdinalMeta(dimInfoList, createInvertedIndices, coordSysInfo) {
	...
	var hasNameEncode;
	coordSysInfo && each$2(dimInfoList, function(dimInfo, dimIndex) {
...
function intervalScaleNiceTicks(extent, splitNumber, minInterval, maxInterval) {
	...
	fixExtent(result.niceTickExtent = [round$1(Math.ceil(extent[0] / interval) * interval, precision), round$1(Math.floor(extent[1] / interval) * interval, precision)], extent);
	return result;
}
...
function fixOnBandTicksCoords(axis, ticksCoords, alignWithLabel, clamp) {
	...
	if (ticksLen === 1) {
		...
		diffSize = 1 + axis.scale.getExtent()[1] - ticksCoords[ticksLen - 1].tickValue;
	...
	*/
	...
	LabelManager.prototype.updateLayoutConfig = function(api) {
		...
		for (var i = 0; i < this._labelList.length; i++) {
			...
			if (isFunction(labelItem.layoutOption)) layoutOption = labelItem.layoutOption(prepareLayoutCallbackParams(labelItem, hostEl));
	...
	LabelManager.prototype.layout = function(api) {
		...
		hideOverlap(filter(labelList, function(item) {
...
function installLabelLayout(registers) {
	registers.registerUpdateLifecycle("series:beforeupdate", function(ecModel, api, params) {
		...
		if (!labelManager) labelManager = getLabelManager(api).labelManager = new LabelManager();
...
var Layer = function(_super) {
	...
	Layer.prototype.clear = function(clearAll, clearColor, repaintRects) {
		...
		else if (repaintRects.length) each$2(repaintRects, function(rect) {
...
var CanvasPainter = function() {
	...
	CanvasPainter.prototype._doPaintList = function(list, prevList, paintAll) {
		...
		if (env.wxa) each$2(this._layers, function(layer) {
...
//#endregion
export { prepareSeriesDataSchema as $, normalizeEvent as $a, calculateTextPosition as $i, getShapeClass as $n, retrieve3 as $o, TSpan as $r, TooltipMarkupStyleCreator as $t, getAxisRawValue as A, parse as Aa, deprecateReplaceLog as Ai, convertToColorString as An, filter as Ao, createFromString as Ar, version as At, getLayoutOnAxis as B, Point as Ba, getPrecisionSafe as Bi, inheritDefaultOption as Bn, isRegExp as Bo, leaveEmphasis as Br, createLegacyDataSelectAction as Bt, GeoSVGRegion as C, round4 as Ca, mappingToExists as Ci, getAvailableSize as Cn, concatArray as Co, Polyline as Cr, registerPreprocessor as Ct, AxisModelCommonMixin as D, lift as Da, queryDataIndex as Di, positionElement as Dn, each$2 as Do, Ellipse as Dr, registerUpdateLifecycle as Dt, helper_exports as E, lerp as Ea, preParseFinder as Ei, mergeLayoutParam as En, defaults as Eo, Sector as Er, registerTransform as Et, shouldShowAllLabels as F, cubicSubdivide as Fa, MAX_SAFE_INTEGER as Fi, windowOpen as Fn, isArrayLike as Fo, enableHoverFocus as Fr, createSymbol as Ft, increaseInterval as G, invert as Ga, parseDate as Gi, getFont as Gn, merge as Go, toggleHoverEmphasis as Gr, createOrUpdate as Gt, createFloat32Array as H, copy as Ha, isRadianAroundZero as Hi, animateLabelValue as Hn, keys as Ho, setDefaultStateProxy as Hr, getVisualFromData as Ht, unionAxisExtentFromData as I, quadraticAt as Ia, asc as Ii, format as In, isDom as Io, enterBlur as Ir, normalizeSymbolOffset as It, OrdinalMeta as J, rotate as Ja, reformIntervals as Ji, setLabelStyle as Jn, noop as Jo, ZRText as Jr, createRenderPlanner as Jt, isIntervalOrLogScale as K, matrix_exports as Ka, parsePercent as Ki, getLabelStatesModels as Kn, mergeAll as Ko, getECData as Kr, throttle as Kt, ensureScaleRawExtentInfo as L, quadraticDerivativeAt as La, getPercentSeats as Li, getLocaleModel as Ln, isFunction as Lo, enterEmphasis as Lr, normalizeSymbolSize as Lt, getScaleExtent as M, createCubicEasingFunc as Ma, makePrintable as Mi, formatTplSimple as Mn, hasOwn as Mo, SPECIAL_STATES as Mr, brushSingle as Mt, ifAxisCrossZero as N, cubicAt as Na, throwError as Ni, normalizeCssArray as Nn, indexOf as No, enableComponentHighDownFeatures as Nr, getLineDash as Nt, createScaleByModel as O, modifyAlpha as Oa, queryReferringComponents as Oi, sizeCalculable as On, eqNaN as Oo, Circle as Or, registerVisual as Ot, niceScaleExtent as P, cubicRootAt as Pa, warn as Pi, toCamelCase as Pn, isArray as Po, enableHoverEmphasis as Pr, getSize as Pt, createDimensions as Q, isMiddleOrRightButtonOnMouseUpDown as Qa, Group as Qi, createIcon as Qn, retrieve2 as Qo, ZRImage as Qr, defaultSeriesFormatTooltip as Qt, TimeScale as R, quadraticSubdivide as Ra, getPixelPrecision as Ri, registerLocale as Rn, isNumber as Ro, isHighDownDispatcher as Rr, symbolBuildProxies as Rt, GeoJSONRegion as S, normalizeColor as Sa, makeInternalComponentId as Si, fetchLayoutMode as Sn, clone$2 as So, Line as Sr, registerPostUpdate as St, createTextStyle as T, fastLerp as Ta, parseFinder as Ti, getLayoutRect as Tn, curry as To, Ring as Tr, registerTheme as Tt, IntervalScale as U, create as Ua, linearMap as Ui, createTextConfig as Un, logError as Uo, setStatesFlag as Ur, setItemVisualFromData as Ut, layout as V, clone as Va, isNumeric as Vi, Model as Vn, isString as Vo, setAsHighDownDispatcher as Vr, getItemVisualFromData as Vt, OrdinalScale as W, identity as Wa, numericToNumber as Wi, createTextStyle$1 as Wn, map$1 as Wo, setStatesStylesFromModel as Wr, clear as Wt, getStackedDimension as X, translate as Xa, round$1 as Xi, applyTransform as Xn, reduce as Xo, parseFontSize as Xr, SERIES_UNIVERSAL_TRANSITION_PROP as Xt, createSeriesData as Y, scale$1 as Ya, remRadian as Yi, setLabelValueAnimation as Yn, normalizeCssArray$1 as Yo, hasSeparateFont as Yr, ComponentView as Yt, isDimensionStacked as Z, addEventListener as Za, zrender_exports as Zi, clipPointsByRect as Zn, retrieve as Zo, Rect as Zr, SeriesModel as Zt, format_exports as _, isGradient as _a, getTooltipRenderMode as _i, SOURCE_FORMAT_OBJECT_ROWS as _n, set$1 as _o, updateProps as _r, registerCoordinateSystem as _t, prepareLayoutList as a, copyTransform as aa, PathProxy as ai, createFilterComparator as an, clone$1 as ao, mergePath as ar, setPlatformAPI as as, connect as at, number_exports as b, isPattern as ba, isNameSpecified as bi, box as bn, assert as bo, CompoundPath as br, registerMap as bt, limitSurfaceAngle as c, adjustTextY$1 as ca, Displayable as ci, DataFormatMixin as cn, dist$1 as co, subPixelOptimizeLine as cr, disConnect as ct, extendChartView as d, getMatrixStr as da, MULTIPLE_REFERRING as di, CoordinateSystemManager as dn, lerp$1 as do, getAnimationConfig as dr, getCoordinateSystemDimensions as dt, getBoundingRect as ea, DEFAULT_PATH_STYLE as ei, buildTooltipMarkup as en, stop as eo, getTransform as er, trim as es, SeriesData as et, extendComponentModel as f, getPathPrecision as fa, SINGLE_REFERRING as fi, getDecalFromPalette as fn, max$1 as fo, getOldStyle as fr, getInstanceByDom as ft, util_exports as g, isAroundZero as ga, getDataItemValue as gi, SOURCE_FORMAT_ARRAY_ROWS as gn, scaleAndAdd as go, saveOldStyle as gr, registerAction as gt, Axis as h, hasShadow as ha, defaultEmphasis as hi, makeSeriesEncodeForNameBased as hn, scale$2 as ho, removeElementWithFadeOut as hr, init as ht, hideOverlap as i, Transformable as ia, containStroke$4 as ii, SortOrderComparator as in, applyTransform$1 as io, makePath as ir, platformApi as is, PRIORITY as it, getDataDimensionsOnAxis as j, stringify as ja, error as ji, formatTpl as jn, find as jo, DISPLAY_STATES as jr, createOrUpdatePatternFromDecal as jt, estimateLabelUnionRect as k, modifyHSL as ka, deprecateLog as ki, capitalFirst as kn, extend as ko, clonePath as kr, setCanvasCreator as kt, limitTurnAngle as l, encodeBase64 as la, createOrUpdateImage as li, normalizeTooltipFormatResult as ln, distSquare as lo, transformDirection as lr, disconnect as lt, extendSeriesModel as m, getShadowKey as ma, convertOptionIdName as mi, makeSeriesEncodeForAxisCoordSys as mn, normalize$1 as mo, removeElement as mr, getMap as mt, install$1 as n, parsePercent$1 as na, normalizeRadian as ni, getPaddingFromTooltipModel as nn, transformLocalCoord as no, groupTransition as nr, DEFAULT_FONT as ns, DataDiffer as nt, shiftLayoutOnY as o, cloneValue as oa, normalizeArcAngles as oi, getRawValueParser as on, copy$1 as oo, setTooltipConfig as or, env as os, dataTool as ot, extendComponentView as p, getSRTTransformString as pa, compressBatches as pi, registerInternalOptionCreator as pn, min$1 as po, initProps as pr, getInstanceById as pt, isValueNice as q, mul as qa, quantile as qi, labelInner as qn, mixin as qo, setCommonECData as qr, ChartView as qt, installLabelLayout as r, TRANSFORMABLE_PROPS as ra, containStroke$2 as ri, retrieveVisualColorForTooltipMarker as rn, Eventful as ro, linePolygonIntersect as rr, DEFAULT_FONT_FAMILY as rs, use as rt, getLabelLineStatesModels as s, TEXT_ALIGN_TO_ANCHOR as sa, fromPoints as si, parseDataValue as sn, create$1 as so, subPixelOptimize as sr, __extends$1 as ss, dependencies as st, install as t, getLineHeight as ta, Path as ti, createTooltipMarkup as tn, encodeHTML as to, graphic_exports$1 as tr, util_exports$1 as ts, getDimensionTypeByAxis as tt, setLabelLineStyle as u, getIdURL as ua, makeStyleMapper as ui, retrieveRawValue as un, len as uo, traverseElements as ur, dispose as ut, graphic_exports as v, isImagePattern as va, groupData as vi, ComponentModel as vn, sub as vo, RadialGradient as vr, registerLayout as vt, contain as w, color_exports as wa, normalizeToArray as wi, getLayoutParams as wn, createHashMap as wo, Polygon as wr, registerProcessor as wt, parseGeoJSON as x, isRadialGradient as xa, makeInner as xi, copyLayoutParams as xn, bind as xo, BezierCurve as xr, registerPostInit as xt, time_exports as y, isLinearGradient as ya, interpolateRawValues as yi, LOCATION_PARAMS as yn, vector_exports as yo, LinearGradient as yr, registerLoading as yt, createProgressiveLayout as z, BoundingRect as za, getPrecision as zi, getUID as zn, isObject$2 as zo, leaveBlur as zr, findEventDispatcher as zt };

//# sourceMappingURL=install-D2aADMtJ.js.map