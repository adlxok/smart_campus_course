<template>
  <div class="dashboard">
    <aside class="sidebar">
      ...
      <nav class="nav-menu">
        <div class="menu-group" v-if="hasAnySystemPermission">
          ...
          <div class="menu-group-items" v-show="expandedGroups.system">
            ...
            <div 
              ...
              v-if="hasPermission('system:category:manage')"
            >
              <span class="icon">📁</span>
              <span>分类管理</span>
            </div>
            <div 
              ...
              v-if="hasPermission('system:user:manage')"
            >
              <span class="icon">👥</span>
              <span>用户管理</span>
            </div>
            <div 
              class="nav-item" 
              :class="{ active: currentMenu === 'roles' }"
              @click="currentMenu = 'roles'"
              v-if="hasPermission('system:role:manage')"
            >
              <span class="icon">🎭</span>
              <span>角色管理</span>
            </div>
            <div 
              class="nav-item" 
              :class="{ active: currentMenu === 'permissions' }"
              @click="currentMenu = 'permissions'"
              v-if="hasPermission('system:permission:manage')"
            >
              <span class="icon">🔐</span>
              <span>权限管理</span>
            </div>
            <div 
              ...
              v-if="hasPermission('system:notification:manage')"
            >
              <span class="icon">🔔</span>
              <span>系统通知管理</span>
            </div>
            <div 
              ...
              v-if="hasPermission('system:banner:manage')"
            >
        ...
        <div class="menu-group" v-if="hasAnyDataPermission">
          ...
          <div class="menu-group-items" v-show="expandedGroups.crawler">
            <div 
              ...
              v-if="hasPermission('data:video:manage')"
            >
              <span class="icon">🎬</span>
              <span>B站视频管理</span>
            </div>
            <div 
              ...
              v-if="hasPermission('data:crawler:manage')"
            >
              <span class="icon">🕷️</span>
              <span>视频爬取</span>
            </div>
            <div 
              ...
              v-if="hasPermission('data:import:manage')"
            >
              <span class="icon">📥</span>
              <span>视频导入</span>
            </div>
            <div 
              ...
              v-if="hasPermission('data:proxy:manage')"
            >
              <span class="icon">🌐</span>
              <span>代理管理</span>
            </div>
            <div 
              ...
              @click="goToAnalysis"
              v-if="hasPermission('data:analysis:manage')"
            >
              <span class="icon">📊</span>
              <span>B站视频数据分析</span>
            </div>
    ...
    <main class="main-content">
      ...
      <div class="content">
        ...
        <div v-if="currentMenu === 'users'" class="user-section">
          <el-card class="user-card">
            <template #header>
              <div class="card-header">
                <span>👥 用户管理</span>
                <el-button type="primary" size="small" @click="openAddUserModal">新增用户</el-button>
              </div>
            </template>
            ...
            <el-table :data="userList" style="width: 100%" v-loading="userLoading">
              ...
              <el-table-column prop="roles" label="角色" min-width="200">
                <template #default="{ row }">
                  <el-tag v-for="role in row.roles" :key="role.id" style="margin-right: 4px" :type="getRoleTagType(role.code)">
                    {{ role.name }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column label="操作" width="280">
                <template #default="{ row }">
                  ...
                  <el-button size="small" type="warning" @click="openUserRoleModal(row)">分配角色</el-button>
          ...
          <el-dialog v-model="showUserModal" :title="isAddUser ? '新增用户' : '编辑用户'" width="500px">
            <el-form :model="userForm" label-width="80px">
              <el-form-item label="用户名">
                <el-input v-model="userForm.username" placeholder="请输入用户名" :disabled="!isAddUser" />
              </el-form-item>
              ...
              <el-form-item label="生日">
                <el-date-picker v-model="userForm.birthday" type="date" placeholder="选择生日" value-format="YYYY-MM-DD" />
              </el-form-item>
          ...
          <el-dialog v-model="showUserRoleModal" title="分配角色" width="500px">
            ...
            <template #footer>
              <el-button @click="showUserRoleModal = false">取消</el-button>
              <el-button type="primary" @click="saveUserRoles">保存</el-button>
            </template>
          </el-dialog>
        </div>

        <div v-if="currentMenu === 'roles'" class="role-section">
            ...
            <el-table :data="roleList" style="width: 100%" v-loading="roleLoading">
              ...
              <el-table-column label="操作" width="280">
                <template #default="{ row }">
                  ...
                  <el-button size="small" type="success" @click="openRolePermissionModal(row)">分配权限</el-button>
                  <el-button size="small" type="danger" @click="deleteRole(row)" :disabled="isSystemRole(row.code)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-card>
          ...
          <el-dialog v-model="showRolePermissionModal" title="分配权限" width="600px">
            <el-form label-width="80px">
            ...
            <template #footer>
              <el-button @click="showRolePermissionModal = false">取消</el-button>
              <el-button type="primary" @click="saveRolePermissions">保存</el-button>
            </template>
          </el-dialog>
        </div>

        <div v-if="currentMenu === 'permissions'" class="permission-section">
  ...
  setup() {
    ...
    const hasAnySystemPermission = computed(() => {
      ...
      const systemPermissions = [
        'dashboard:view',
        'system:category:manage',
        'system:user:manage',
        'system:role:manage',
        'system:permission:manage',
        'system:notification:manage',
        'system:banner:manage'
      ]
      return systemPermissions.some(p => userPermissions.value.includes(p))
    })
    
    const hasAnyDataPermission = computed(() => {
      if (userRole.value === 'SUPER_ADMIN') return true
      ...
      return dataPermissions.some(p => userPermissions.value.includes(p))
    })
    ...
    const saveUser = async () => {
      try {
        if (isAddUser.value) {
          const response = await api.post('/admin/user/add', userForm.value)
          if (response.data.success) {
          ...
          const response = await api.put(`/admin/user/${userForm.value.id}`, userForm.value)
          if (response.data.success) {
    ...
    const openUserRoleModal = async (user) => {
    ...
    const isSystemRole = (code) => {
      return ['SUPER_ADMIN', 'SYSTEM_ADMIN', 'DATA_ADMIN', 'USER'].includes(code)
    }

    const openRolePermissionModal = async (role) => {
    ...
    watch(currentMenu, (newMenu) => {
      const systemMenus = ['dashboard', 'categories', 'users', 'roles', 'permissions', 'notifications', 'banners']
